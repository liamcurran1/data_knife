CREATE OR REPLACE FUNCTION public.data_extract(list text, dbname text)
 RETURNS text
 LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  -- This version lower cases fieldname before it tests to see if present in table of basket variable names.
  OPEN mark FOR EXECUTE 'SELECT * FROM (SELECT distinct on (fieldname)  database, schemaref, fieldname, tableref, numrows FROM '
                         || dbname ||'.public.dat_index ' ||
	                'WHERE lower(fieldname) IN (SELECT * FROM get_list(''' || list || ''')) order by fieldname, numrows DESC) AS foo ORDER BY numrows DESC,tableref';
  <<process_loop>>
  LOOP
